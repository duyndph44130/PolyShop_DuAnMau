...
class DashboardModel {
    ...
    public function getStats() {
        $stmt = $this->conn->query("
            SELECT 
                ...
                (SELECT SUM(total_amount) FROM orders WHERE status = 'completed') AS revenue
        ");
    ...
    public function getRecentUsers() {
        return $this->conn->query("SELECT name, email FROM user ORDER BY user_id DESC LIMIT 3")->fetchAll(PDO::FETCH_ASSOC);
    }
